'''
    ...
    Each intra_clustering() call from intra_blob() adds a layer of sub_blobs, new dert to derts and Layer to Layers, in each blob.
    intra_clustering also sends feedback to fork[fia][fder] in root_blob, then to root_blob.root_blob, etc.
    Blob structure:
    ...
    derts__,  # intra_clustering inputs
...
ave = 20   # average g, reflects blob definition cost, higher for smaller positive blobs, no intra_clustering for neg blobs
ave_blob = 10000       # fixed cost of intra_clustering per blob, accumulated in deeper layers
rave = 20              # fixed root_blob / blob cost ratio: add sub_blobs, Levels+=Level, derts+=dert
ave_n_sub_blobs = 10   # determines rave, adjusted per intra_clustering
ave_intra_blob = 1000  # cost of default eval_sub_blob_ per intra_blob

''' These filters are accumulated for evaluated intra_clustering:
...
"""
def intra_blob(root_blob, rng, Ave_blob, Ave):  # Single blob recursive forking:

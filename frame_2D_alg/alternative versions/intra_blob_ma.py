...
def cluster_derts(dert__, Ave, fcr, fig):  # similar to frame_to_blobs
    ...
    if fcr:  # comp_r output;  form clustering criterion:
        if fig:
            crit__ = dert__[0] + dert__[4] - Ave  # eval by i + m, accum in rng; dert__[:,:,0] if not transposed
        else:
            crit__ = Ave - dert__[1]  # eval by -g, accum in rng
    else:  # comp_g output
...
def extend_dert_diag(blob, ext_num=1, unmask_ext=1, diag=1):
    ...
    yne = yn + ext_num;
    x0e = x0 - ext_num;
    xne = xn + ext_num  # e is for extended
    # prevent boundary <0 or >image size:
    if y0e < 0:
        y0e = 0;
        ystart = 0

...
if __name__ == '__main__':
    ...
    if intra:  # Tentative call to intra_blob, omit for testing frame_blobs:
        ...
        for i, blob in enumerate(frame['blob__']):  # print('Processing blob number ' + str(bcount))
            ...
            # predictive value borrow from adjacent flat (-G) blobs to edge (+G) blobs is proportional to relative min_G:
            borrow_G = min(G, adj_G) * (1 - (min(G, adj_G) / max(G, adj_G)))
            min(G, adj_G) * (min(G, adj_G) / max(G, adj_G))

            # full adj_G -> G, part G -> adj_G: each lends to other blobs?
            # if env is closed in 2D? some cross-induction or even then?

...
def inc_deriv(blob):  # compute and compare angle, define ablobs, accumulate a, da, sda in all reps within gblob
...
def comp_g(y, dert_, lower_dert_, P_map, lower_P_map):
    ...
    gdx_ = np.zeros(g_.shape, dtype=int)
    gdx_[comp_map] = right_g_[comp_map] - g_[comp_map]
    gdy_ = np.zeros(g_.shape, dtype=int)
    gdy_[comp_map] = lower_g_[comp_map] - g_[comp_map]

    gg_ = np.hypot(gdy_, gdx_) - ave

    dert_[:-1, -3] = gg_  # assign gg_ to a slice of dert_
    dert_[:-1, -2] = gdx_ # assign gdx_ to a slice of dert_
    dert_[:-1, -1] = gdy_ # assign gdy_ to a slice of dert_

...
Each clustering cycle has 4 incrementally sparse and fuzzy stages of scope expansion, seeded by prior-stage output:
...
- add EM-like function extending cluster_P to adjacent frames, from projected C-to-N links, not implemented.
...
ttN_,ttC_,ttN,ttF, ttE,ttcN,ttcC,ttcP, ttX,ttFrm,ttVct,ttTrc,ttBac,ttPrj = FTT_
...
class CoF(CF):  # oF/ code fork, N_,dTT: data scope, w = vt_(wTT)[0]?
    ...
    def traced(func):
        ...
        def inner(*a, **kw):
            ...
            if oF.nF == 8:  # cross_comp
                tile = a[0].root  # a is Ft
                tile.wTT_ = deepcopy(FTT_); add_typ_(oF)
                for typ in oF.typ_:
                    tile.wTT_[typ.nF] =  cent_TT(getattr(typ,'rTT', typ.dTT),typ.r)  # compute wTT
            return out
        inner.wrapped = True
        return inner
...
def comp_F(_F, F, ir=0, rL=None):
    ...
    if _F.nF == F.nF:  # sub-comp
        ...
        if  _N_ and N_:
            ...
            if (rL.m+m)/2 * ((len(Np_)-1)*Lw) * wF > ave * cF:
...
def cluster_N(Ft, _N_, r,_c):  # flood-fill node | link clusters, flat, replace iL_ with E_?
    ...
    if _N_ and val_(Ft.dTT, r+ccN, Ft.root.wTT*ttcN, mw=(len(_N_)-1)*Lw *wcN) > 0:  #| fL?
        for N in _N_:
        ...
        for N in _N_:  # form G per remaining N
            ...
            if N_:
                ...
                if val_(tt, r, Ft.root.wTT*ttcN, (len(N_)-1)*Lw) > 0:  # apply Fw_ and Fc_ in every eval_?
        ...
        if G_:
            ...
            if val_(TT, r+R, Ft.root.wTT*ttcN, (len(G_)-1)*Lw) > 0:  # reform root,Nt, no other forks yet:
...
def cluster_C(Ft, E_, r,_c):  # form centroids by clustering exemplar surround via rims of new member nodes, within root
        ...
        for cr, (_C,_U,rc) in enumerate(sorted(_Ct_, key=lambda t: t[1]*t[2], reverse=True),start=1):  # rc weighting?
            if _U > Ave:
                ...
                for n in set(_C.N_+_C._N_):  # current + frontier
                    ...
                    if oL_: _m,_d = sum_vt(oL_,fm=1, fdiv=1, wTT=ttcC)[:2]; u+=_m*n.c; d+=_d
                    u_+=[u]; U+=u; D+=abs(d)  # from all comps, U is not c-weighted?
                    if u/U > Ave:  # only normalize for eval?
                        N_+=[n]; L_+=n.L_  # convergence val
                ...
                if U > Ave and val_(dTT, cr,_C.wTT*ttcC,(len(N_)-1)*Lw) > 0:
                    C =  sum2C(N_,u_, root=Ft, wTT=ttcC)
    ...
    if oC_:
        ...
        if not fP: oC_ = prune_C_(oC_, u__, Ft, ttcC)
...
def cluster_P(__C_, N_, _c, root):  # FCM-style parallel centroid refine, may add proj_C
    ...
    while True:
        u__, dU = [], 0
        C_ = [sum2C(N_, u_, root, ttcP) for u_ in _u__]  # same N_ * updated membership u_
    ...
    out_ = prune_C_(C_, u__, root, ttcP)
    ...
    for C,_u_ in zip(C_,u__):
        if C.m > ave * C.r:  # final pruning, C vals are competitive
            ...
            if N_:  # assign C.u_, N.rN_
                out_ += [sum2C(N_, u_,root=root, wTT=wTT, final=1)]
    return out_
...
def sum2F(N_, root, fr=0, merge=0, froot=0):  # -> CF/CN
    ...
    if getattr(N_[0], 'wTT_', None):  # summing tile into lev
        root.wTT_ = np.sum([N.wTT_ for N in N_],axis=0)  # or sum oF's wTT|dTT, then recompute it here?
    return root
...
def sum2C(N_, u_, root=None, wTT=wTT, final=0, fP=0):  # fuzzy sum + base attrs for centroids

    L_, uc_ = [],[]  # N/C contribution = c-scaled u_
    for i, N in enumerate(N_):
        if final: L_ += [CN(typ=1, w=[N.c * u_[i]])]  # add u,c,r,TT,span,angl for proj_C?
...
def sum2G(ft_, root=None, init=1, typ=None):
    ...
    if G.Lt:  # sub+
        ...
        if lm*ld* ((len(L_)-1)*Lw) > ave*avd* (lr+1+ccN):
            V = lm - ave*(lr+1+ccN); c = G.Lt.c
            if V > 0:
                r+=1; E_= get_exemplars({N for L in L_ for N in L.nt}, r,c)
                G_,r= cluster_C(G.Nt,E_,r,c) if (mdecay(L_)-decay)*V > ave*ccC else cluster_N(G.Nt, E_,r,c)
                if G_ and val_(G.Nt.dTT, r+cX, G.wTT*ttX, mw=(len(G_)-1)*Lw) > 0:
                    cross_comp(G.Nt,r,'Nt')
    if G.Bt:
        Bt = G.Bt; bd,br = Bt.d,Bt.r; rroot = root.root if root.root else 0
        if bd > avd*br*cX and N.typ!=1: cross_comp(Bt, br,'Bt')  # no ddfork
    ...
    for blob in blob_:
        if not blob.sign and blob.G > aveB:
            ...
            if edge.G * ((len(edge.P_)-1)*Lw) > ave * sum([P.latT[4] for P in edge.P_]):
                ...
                if val_(sum_vt(N_,wTT=ttVct)[0],3, tile.wTT*ttVct, (len(PPm_)-1)*Lw) > 0:
...
def trace_edge(N_,_G_,_TT,_C, r,root):  # cluster contiguous shapes via PPs in edge blobs or lGs in boundary/skeleton?
    ...
    if val_(TT,r+1, root.wTT*ttTrc) > 0: _G_+=G_; _TT+=TT;_C+=C  # eval per edge, concat in tile?
...
def ffeedback(frame,c,r):  # adjust filters: all aves *= rV, ultimately differential backprop per ave?
    '''
    future code consolidation, for all onF_:
    ...
    _wTT_ = frame.H[0].wTT_  # assign per level?
    for lev in frame.H[1:]:
        # sum ratios between consecutive-level TTs, top-down frame expansion levels, not lev-selective or sub-lev recursive

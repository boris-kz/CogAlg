...
class CN(CBase):
    ...
    def __init__(n, **kwargs):
        ...
        n.L_ = kwargs.get('L_',[])  # internal links if fi else len nodet.N_s?
        n.Et = kwargs.get('Et',np.zeros(3))  # sum from L_
        n.rim= kwargs.get('rim',[])  # external links, rng-nested?
        ...
        n.sub  = 0  # max composition depth relative to top composition graphs
...
def cross_comp(root, rc, fC=0):  # rng+ and der+ cross-comp and clustering
    ...
    if len(L_) > 1:
        ...
        if mV > 0:
            nG = Cluster(root, N_, rc+O, fC)  # get_exemplars, cluster_C, rng connectivity cluster
            ...
            for dist, dy_dx, _N in N.pL_:
                ...
                if V >= 0:  # tentative
                    ...
                    if adist * V/olp > dist:
                        ...
                        pVt_ += [[dist, dy_dx, _N, val_(Link.Et, aw=rc+olp)]]
                        # per compared N
    sL_ = sorted(L_, key=lambda l: l.span)
    L_, Lseg = [], [L_[0]]
    for _L, L in zip(sL_, sL_[1:]):  # segment by ddist:
...
def comp_N(_N,N, olp,rc, A=np.zeros(2), span=None, rng=1, lH=None):  # compare links, optional angl,span,dang?
    ...
    if fi and V > ave * rc+1 + compw:
        if N.L_: spec(_N.N_, N.N_, Et, olp+rc+1,Link.lH)  # skip PP
        if _N.B_ and N.B_:
            _B_,_bEt,_bO = _N.B_; B_,bEt,bO = N.B_
            if val_(_bEt+bEt,1,(min(len(_B_),len(B_))-1)*Lw, bO+bO+rc+compw) > 0:
                spec(_B_,B_,_bEt+bEt,_bO+bO, Link.lH)  # lH = dspe; spec C_: overlap, offset?
    if lH is not None:
        lH += [Link]
...
def cluster_n(root, iC_, rc, rng=1):  # simplified flood-fill, currently for for C_ only
...
def ett(L): return (L.N_[0].et + L.N_[1].et - L.Et*2) * intw + L.Et  # L.Et is twice included in ett
...
def sum2graph(root, N_,L_,C_,B_,long_, Et,olp,rng):  # sum node,link attrs in graph, aggH in agg+ or player in sub+
    ...
    graph = CN(root=root, fi=1,rng=rng, N_=N_,L_=L_,C_=C_,B_=B_, Et=Et,rc=olp, baseT=baseT, derTT=derTT, derH=derH, span=span, angl=A, yx=yx)
    ...
    for blob in tile.N_:  # blobs is packed in N_ now
        if not blob.sign and blob.G > aveB:
            ...
            if edge.G * ((len(edge.P_)-1)*Lw) > ave * sum([P.latT[4] for P in edge.P_]):
                ...
                form_B__(Fg,CN(N_=PPd_),2)
                if val_(Et, mw=(len(PPm_)-1)*Lw, aw=2) > 0:  # internal B_
                    trace_edge(Fg, rc=2, fN_=1)  # contiguous boundary-mediated xcomp,cluster of complemented Ns
                    # trace_edge(Fg, rc=3, fN_=0)?
                Fg_ += [Fg]
    return sum_N_(Fg_)
...
def trace_edge(root, rc, fN_):  # cluster contiguous shapes via PPs in edge blobs or lGs in boundary / skeleton?

    N_ = root.N_ if fN_ else root.B_[0]  # fB_: cluster B_-mediated N_, else rB_-mediated B_

...
def vt_(TT):  # brief val_ to get N.m, N.d
...
def cross_comp(root, rc, fC=0):  # rng+ and der+ cross-comp and clustering, fT: convert return to tuple
    ...
    if val_(mTT, rc+contw, mw=(len(root.N_)-1)*Lw) > 0:  # mval only
        fnG = trace_edge(root,rc); rc+=fnG  # comp Ns x N.Bt|B_.nt, with/out mfork?
    if fnG and val_(root.dTT, rc+compw, mw=(len(root.N_)-1)*Lw, _TT=mTT) > 0:  # root.N_ compressed in trace_edge
        _,rc = cross_comp(root, rc)
        # connect agg+, fC=0
    return fnG or flG, rc
    ...
    for _F_,F_,nF_,nFt in zip((_N.N_,_N.B_,_N.C_), (N.N_,N.B_,N.C_), ('N_','B_','C_'),('Nt','Bt','Ct')):  # + tN_,tB_,tC_ from trans_cluster?
        if _F_ and F_:
            ...
            if dF_:
                ...
                Rc += 1
    if _N.Nt and N.Nt:  # no comp Bt,Ct: external to N,_N
        ...
        for _lev,lev in zip(_H,H):
            ...
            if _lev.Nt.N_ and lev.Nt.N_ and m > ave * Rc:  # lev nesting incr / depth
                comp_sub(_lev,lev, Rc,dlev)  # dlev+=dsub
...
def Copy_(N, root=None, init=0, typ=None):  # not updated
...
def form_B__(G):  # assign boundary / background per node from Bt, no root update?
    ...
    for bG in Bt.N_:  # add R_ per boundary graph, in Fg?
        ...
        bG.rB_=sorted(R_, key=lambda x:(x.m/x.c), reverse=True)
    def R(L):
        root = L.root
        if root:
...
def sum_N_(N_,rc, root=None, L_=[],C_=[],B_=[], rng=1, init=1):  # updates root if not init
    ...
    if init:  # new cluster
        G = CN(nest=root.nest if root else 1, N_=N_,L_=L_,B_=B_,C_=C_, rc=rc, rng=rng, root=root)  # new cluster
    else:   # extend root
    ...
    for i, (nFt,F_) in enumerate(zip(('Nt','Lt'),(G.N_,G.L_))):
        ...
        for F in F_:
            if i: A += F.angl[0]  # F is link
                ...
                add_N(G, F)  # add not-Ft attrs only
                if init: yx_ += [F.yx]  # or weigh contributions by c in a add_N?
            for Lev,lev in zip_longest(H, F.Nt.N_, fillvalue=[]):
                if lev:
                    if Lev: Lev += lev.N_
                    else:   H += [lev.N_]
...
def sum2T(N_, rc, root, TT=None, c=1, fcore=1):  # simplified for Ft
    ...
    if N.typ<3:  # not PP
        for nFt, nF_, F_ in zip(('Nt','Bt','Ct','Lt'),('N_','B_','C_','L_'), (Nt.N_, Nt.B_,Nt.C_,Nt.L_)):  # add tFs?
            if F_:
                F_[:] = list(set(F_))
                Ft = sum2T(F_, rc,root, fcore = nFt==('Nt' or 'Lt')); setattr(Nt,nFt,Ft)
                if not fcore: root_update(Nt,Ft)  # else updated in add_N
...
def add_N(N, n):
    ...
    if N.typ >1:  # nodes
        ...
        for L in n.rim:  # may be redundant?
            if L.m > ave: N.L_ += [L]
            else: N.B_ += [L]
...
def PP2N(PP):
    ...
    PP = CN(typ=3, N_=P_,L_=L_,B_=B_, m=m,d=d,c=c, baseT=baseT, dTT=dTT, box=box, yx=yx, angl=A, span=np.hypot(dy/2, dx/2))
    for P in P_: P.root = PP
    # empty N_, PP.Nt = CN(typ=0, dTT=dTT, m=m,d=d,c=c); PP.Lt = CN(typ=0, dTT=sum(l.vert for l in link_))
    return PP
...
def trace_edge(root, rc):  # cluster contiguous shapes via PPs in edge blobs or lGs in boundary/skeleton?
    ...
    for n_,l_, dtt,olp, merged in Gt_:
        if not merged:
            if val_(dtt,rc,mw=(len(n_)-1)*Lw) > 0:

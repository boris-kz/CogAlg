...
def cross_comp(root, rc, fC=0):  # rng+ and der+ cross-comp and clustering, fT: convert return to tuple
    ...
    if len(mL_)>1 and val_(mTT, rc+compw, mw=(len(mL_)-1)*Lw) > 0:
        ...
        root.Lt = Lt; root.Nt = sum_N_(N_,rc, root)  # update Nt
...
def root_update(root, T):

    _c,c = root.c,T.c; C =_c+c; root.c=C  # c is not weighted
    dTT,rC = root.dTT, root.rc; dtt,rc = T.dTT,T.rc
    root.dTT = (dTT*_c + dtt*c) /C; root.rc = (rC*_c + rc*c) /C
...
def form_B__(G):  # assign boundary / background per node from Bt, no root update?
    ...
    for bG in Bt.N_:  # add R_ per boundary graph, in Fg?
        ...
        bG.rB_=sorted(R_+[G], key=lambda x:(x.m/x.c), reverse=True)  # Nt.N_ = R_ if Nt.nest=0
    def R(L):
        return L.root if L.root is None or L.root in Bt.rB_ else R(L.root)
    ...
    for N in G.N_:
        ...
        for L in N.B_:
            ...
            if bG and bG not in bG_:
                ...
                # n stronger cores of rB
        N.Bt = CN(typ=0, dTT=dTT,m=sum(dTT[0]),d=sum(dTT[1]), c=sum(b.c for b in N.B_),rc=rdn, root=N)
    G.Bt = Bt
...
def comp_N_(iN_,rc,_iN_=[]):
    ...
    def proj_V(_N,N, dist, Ave, pVt_):  # _N x N induction
        ...
        if abs(V) > Ave: return V  # +|-
            ...
            for _dist,_dy_dx,__N,_V in pVt_:
                eTT += proj_N(N,_dist,_dy_dx, rc)  # if pV>0: eTT += pTT  # +ve only?
    ...
    for N in iN_:
        ...
        for dist, dy_dx, _N in N.pL_:  # rim angl not canonic
            ...
            if (1-abs(V)) > Ave:
                # projected surprise value: comp if marginal predictability
            ...
            for i, L in enumerate(link_):
                if val_(L.dTT,rc+compw, fi=0) < 0:  # merge
                    ...
                    if _N is not N:  # not merged
                        add_N(_N,N,froot=1, merge=1)  # merge Cs, set fin,root.rim
    ...
    for C in iC_:  # form G per remaining C
        ...
        if node_:
            ...
            if val_(dTT,rc+olp, mw=(len(N_)-1)*Lw,_TT=root.dTT) > 0:
                ...
                G_ += N_
    if G_:
        sum_N_(G_, rc, root, init=0); up=1  # updates root.N_ and root.Nt internally
    return up  # higher root.N_
...
def cluster_C(E_, root, rc):  # form centroids by clustering exemplar surround via rims of new member nodes, within root
    ...
    while True:
        ...
        for _C,_m,_o in sorted(_Ct_, key=lambda t: t[1]/t[2], reverse=True):
            if _m > Ave * _o:
                C = cent_attr( sum_N_(_C.N_, rc, root), rc)  # C update lags behind N_; non-local C.rc += N.mo_ os?
                ...
                DTT += dTT
                if M > Ave * len(_N_) * O:
                    for n, mo in zip(_N_,mo_): n.mo_+=[mo]; n.rC_+=[C]; C.rC_+=[n]  # reciprocal root assign
...
def Copy_(N, root=None, init=0):  # not updated
...
def add_N(N, n, froot=0, merge=0):
    ...
    if merge:  # for levels and C_
        for L_,l_ in zip((N.N_,N.B_,N.C_,N.L_), (n.N_,n.B_,n.C_,n.L_)):
            L_ += l_ if fC else []+l_  # concat, all maybe empty, shift down nt.H?
    if N.typ:
        ...
        for Ft,ft,par in zip((N.Bt, N.Ct, N.Nt, N.Lt), (n.Bt, n.Ct, n.Nt, n.Lt),('Bt','Ct','Nt','Lt')):
            if ft:
                if Ft: add_N(Ft, ft)
    ...
    def expand_lev(_iy,_ix, elev, Fg):  # seed tile is pixels in 1st lev, or Fg in higher levs
        ...
        while True:
            ...
            if Fg and val_(Fg.dTT, Fg.rc+compw+elev, mw=(len(Fg.N_)-1)*Lw) > 0:
                ...
                if 1- abs(val_(pTT, rc=elev) > ave):  # search if marginal predictability

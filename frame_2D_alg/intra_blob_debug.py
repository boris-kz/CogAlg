...
def intra_blob_hypot(frame):  # evaluates for hypot_g and recursion, ave is per hypot_g & comp_angle, or all branches?
    ...
    for blob in frame.blob_:
        if blob.Derts[-1][-1] > ave_root_blob:  # G > root blob conversion cost
            ...
            blob.inp = None  # no comp redefines g as hypot(dy, dx), adds Dert & sub_blob_, calls recursive intra_blob
            intra_comp(blob, hypot_g, ave_root_blob, ave)
...
def intra_blob(root_blob, Ave_blob, Ave, rng):  # recursive intra_comp(comp_branch) selection per branch per sub_blob
    ...
    for blob in root_blob.sub_blob_:
        if blob.Derts[-1][-1] > Ave_blob + ave_eval:  # noisy or directional G: > root blob conversion cost
            ...
            blob.inp = None  # no i comp, angle calc & comp, no eval, Ave_blob return, else intra_blob call
            blob.rng = 1
            Ave_blob = intra_comp(blob, comp_angle, Ave_blob, Ave)
            ...
            for ablob in blob.sub_blob_:  # ablobs are defined by the sign of ga: gradient of angle
                ...
                vals = sorted((
                    ...
                    (val_gr, Ave_blob*2, comp_range), (-1, -(rng*2+1)), rng), key=lambda: [0], reverse=True)
                ...
                for val, threshold, comp_branch, inp, rng in vals:
                    ...
                    if val > threshold * rdn:
                        ...
                        blob.inp = inp
                        blob.rng = rng
                        intra_comp(ablob, comp_branch, Ave_blob, Ave)  # adjusts Ave_blob, evaluates for intra_blob
                    else:
                        break
    ''' 
    the above expanded:

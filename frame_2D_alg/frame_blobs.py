...
def form_P_(y, dert__):  # horizontally cluster and sum consecutive pixels and their derivatives into Ps
    ...
    L, I, Dy, Dx, G = 1, i, dy, dx, g   # P params
    P_dert_ = [(y, 1, i, dy, dx, g)]    # dert buffer
    _s = g > 0                          # sign

    for x, (i, dy, dx, g) in enumerate(dert_[2:-1], start=2):
        s = g > 0
        if s != _s:                     # P is terminated and new P is initialized
            P_.append([_s, L, I, Dy, Dx, G, P_dert_])
        ...
        Dx += dx
        G += g
        P_dert_.append((y, x, i, dy, dx, g))
        _s = s  # prior sign

    P_.append([_s, L, I, Dy, Dx, G, P_dert_])   # last P in row
    return P_
    ...
    while P_:
        ...
        if not fork_:  # seg is initialized with initialized blob
            ...
            if len(fork_) == 1 and fork_[0][3] == 1:  # P has one fork and that fork has one root
                seg = fork_[0]
                L, I, Dy, Dx, G = params
                Ly, Ls, Is, Dys, Dxs, Gs = seg[1]  # fork segment params, P is merged into segment:
                seg[1] = [Ly + 1, Ls + L, Is + I, Dys + Dy, Dxs + Dx, Gs + G]
                seg[2].append(P)  # Py_: vertical buffer of Ps
                seg[3] = 0  # reset roots
                blob = seg[-1]
...
def form_blob(term_seg, frame):  # terminated segment is merged into continued or initialized blob (all connected segments)
    ...
    s, params, Py_, roots, fork_, blob = term_seg
    ...
    if not blob[3]:  # if open_segments == 0: blob is terminated and packed in frame
        ...
        s, [Ly, L, I, Dy, Dx, G], seg_, open_segs, box = blob
        y0, x0, xn = box
        yn = Py_[-1][-1][0][0]  # last P
        Y, X = 0, 0
...
nt_blob = namedtuple('blob', 'typ sign Y X Ly L Derts seg_ sub_blob_ sub_Derts layer_f add_dert map box rng')

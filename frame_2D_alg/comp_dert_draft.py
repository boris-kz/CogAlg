...
if fa: compute and compare angle from dy, dx in dert[-2, -1] (g_dert or ga_dert), only in 2nd intra_comp of intra_blob
else:  compare input param in dert[ia]: p|g in derts[cyc][0] or angle a in dert[1]

primary or recycled flag_ga: i_dert = derts[cyc][fga], separate from flag_ia:
primary or recycled flag_ia: i = dert[fia]: selects dert[1] for incremental-range comp angle only?
'''
...
def compare_derts(P_, buff___, Ave, rng, fga, fia, fa=0, hg=0):  # dert_buff___ in blob ( dert__ in P_ line ( dert_ in P
    ...
    if hg:
        ...
        _derts__ = vertical_comp(derts__, buff___, rng, ia, fa)  # vertical and diagonal comp, returns last line in buff___
...
def lateral_comp(P_, rng, fga, fia, fa=0):  # horizontal comparison between pixels at distance == rng
    ...
    cyc = -rng - 1 - fa   # cyc and rng are cross-convertible, [cyc][fa]: index of input dert and feedback Dert_forks
    ...
    for P in P_:
        ...
        for derts in derts_:
            ...
            i = derts[cyc][-fa][fa]  # input is brightness or gradient in dert[0] or angle in dert[1]
            ...
            if len(buff_) == rng:          # xd == rng and coordinate is within P vs. gap
                ...
                _i = _derts[cyc][-fa][fa]  # template is brightness or gradient in dert[0] or angle in dert[1]
...
def vertical_comp(derts__, buff___, rng, ia, fa):    # vertical and diagonal comparison

    out_derts__ = []     # initialize output
    # first line of derts in last element of buff___ is returned to comp_dert() at len = maxlen(rng)

    yd = 1
    cyc = -rng - 1 - ia  # cyc and rng are cross-convertible, [cyc][fa] is index of input dert and feedback Dert_forks
    ...
    for index, _derts__ in enumerate(buff___):  # iterate through (rng - 1) higher lines
        if yd < rng:  # diagonal comp, else rng == 1?
            ...
            _derts__, derts__ = scan_slice_diag(_derts__, derts__, shift, coefs, cyc, ia, fa)

            buff___[index] = _derts__ # return

        else:   # strictly vertical comp, no shift, fixed coef
            out_derts__, derts__ = scan_slice_(_derts__, derts__, cyc, ia, fa)  # _derts__ are converted to out_derts__
...
def scan_slice_(_derts__, derts__, cyc, ia, fa=False):     # unit of vertical comp
        ...
        while i_derts_ < len(_derts__):
            ...
            if i_derts_ < len(_derts__) and  _x0 < xn:   # if overlap, compare slice:
                ...
                for _derts, derts in zip(_derts_[_start:_end], derts_[start:end]):
                    ...
                    i = derts[cyc][-fa][ia]  # input is brightness or gradient in dert[0] or angle in dert[1]
                    dy, dx = derts[-2, -1]   # derivatives accumulated in input dert over shorter + current rng comps

                    _i = _derts[cyc][-fa][ia]  # template is brightness or gradient in dert[0] or angle in dert[1]
                    _dy, _dx = _derts[-2, -1]  # derivatives accumulated in template dert over shorter + current rng comps

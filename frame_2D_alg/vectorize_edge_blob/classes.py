...
class Cvec2d(NamedTuple):
    ...
    def __add__(self,
                other): return self + other  # .__class__(self.dy + other.dy, self.dx + other.dx) ?
...
class Cptuple(Ct):
    ...
    def __neg__(self): return self.__class__(-self.I, -self.G, -self.M, -self.Ma, -self.angle,
                                             -self.L)

    def comp(self, other, rn):
        dI = self.I - other.I * rn;
        mI = ave_dI - dI
        dG = self.G - other.G * rn;
        mG = min(self.G, other.G * rn) - aves[1]
        dL = self.L - other.L * rn;
        mL = min(self.L, other.L * rn) - aves[2]
        dM = self.M - other.M * rn;
        mM = get_match(self.M, other.M * rn) - aves[3]  # M, Ma may be negative
        dMa = self.Ma - other.Ma * rn;
        mMa = get_match(self.Ma, other.Ma * rn) - aves[4]
        mAngle, dAngle = self.angle.comp_angle(other.angle)

        mtuple = Ct(mI, mG, mM, mMa, mAngle - aves[5], mL)
        dtuple = Ct(dI, dG, dM, dMa, dAngle, dL)
        tuplet = Ct(mtuple, dtuple)
...
class CderH(CBase):  # derH is a list of der layers or sub-layers, each = ptuple_tv
    ...
    def __add__(self, other):
        ...
        H = [Dertuplet + dertuplet for Dertuplet, dertuplet in
             zip_longest(self.H, other.H, fillvalue=self.empty_layer())]  # mtuple,dtuple
    ...
    def comp(self, other, rn, n=inf):
        ...
        for i, _lay, lay in zip(count(), self,
                                other):  # compare common lower der layers | sublayers in derHs
            if i >= n: break
...
class CP(
    CBase):  # horizontal blob slice P, with vertical derivatives per param if derP, always positive
    ...
    cells: set = z(
        set())  # pixel-level kernels adjacent to P axis, combined into corresponding derts projected on P axis.
...
class CderP(
    CBase):  # tuple of derivatives in P link: binary tree with latuple root and vertuple forks
    ...
    def comp(self, link_, rn):
        ...
        if valt.m > ave_Pd * rdnt.m or valt.d > ave_Pd * rdnt.d:
            self.derH |= dderH;
            self.valt = valt;
            self.rdmt = rdnt  # update derP not form new one
            link_ += [self]
...
class Cgraph(CBase):  # params of single-fork node_ cluster
    ...
    aggH: list = z(
        [])  # [[subH,valt,rdnt,dect]], subH: [[derH,valt,rdnt,dect]]: 2-fork composition layers
    valt: Ct = z(Ct([0, 0]))  # sum ptuple, derH, aggH
    rdnt: Ct = z(Ct([1, 1]))
    dect: Ct = z(Ct([0, 0]))
    link_: list = z([])  # internal, single-fork, incrementally nested
    node_: list = z(
        [])  # base node_ replaced by node_t in both agg+ and sub+, deeper node-mediated unpacking in agg+
    # graph-external, +level per root sub+:
    rimH: list = z(
        [])  # direct links, depth, init rim_t, link_tH in base sub+ | cpr rd+, link_tHH in cpr sub+
    ...
    edect: list = z([0, 0])
    ext: list = z([0, 0, Ct([0,
                             0])])  # L,S,A: L len base node_, S sparsity: average link len, A angle: average link dy,dx
    rng: int = 1
...
def get_match(_par, par):
    ...
    return -match if (_par < 0) != (
                par < 0) else match  # match = neg min if opposite-sign comparands

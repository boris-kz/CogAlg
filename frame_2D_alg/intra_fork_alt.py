...
from comp_i import (
    comp_i, F_DER_A, F_DERIV, F_RANGE,
)
...
def intra_fork(fork, flags, Ave_blob, Ave, rdn):
    ...
    compare fork gradients, or compare angles of fork gradients)
    """
    ...
    if sub_fork['G'] > ave_intra_blob * rdn:
        intra_fork(sub_fork, F_DER_A, Ave_blob, Ave, rdn+1)
        intra_fork(sub_fork, F_DERIV, Ave_blob, Ave, rdn+1)

    if (not flags & F_DER_A # Angle fork don't have F_RANGE sub-fork
...
def intra_clustering(root_blob, fork, Ave, Ave_blob, fa):
    ...
    P__ = form_P__(dert__, Ave, fa, ncomp=((2*fork['rng'] + 1)**2-1), x0, y0) # Horizontal clustering
    P_ = scan_P__(P__)
    seg_ = form_segment_(P_)
    blob_ = form_blob_(seg_, root_blob, fork, fa)

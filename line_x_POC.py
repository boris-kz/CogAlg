    ...
    if ( x > rng * 2 and s == pri_s ) or x == X-1:  # m sign change, mP is terminated and evaluated for recursive comp
        if pri_s:  # forms sub_mP_ within positive mP
            ...
            for i in range(L + 1):
                ip, id, im = e_[i]
                sd = 1 if id > 0 else 0
                pri_sd, Ld, Id, Dd, Md, sr, se_ = dP
                if ( pri_sd != sd and not i == 0 ) or i == L:
                    if Ld > rng + 3 and Md < ave_M * rdn:   # comp range increase within e_:
                        sr = 1                              # rdn: redundancy, incremented per comp recursion
                        rng += 1
                        sub_mP_= []; sub_mP = 0,0,0,0,0,0,[]  # pri_s, L, I, D, M, r, e_;  no Alt: M is defined through abs(d)

                        for j in range(rng, Ld):  # comp between rng-distant pixels, also bilateral, if L > rng * 2?
                            ipd, fd, fm = se_[j]
                            _ip, _fd, _fm = se_[j - rng]
                            ed = ipd - _ip  # ed: element d, em: element m:
                            em = min(ipd, _ip) - ave_m  # magnitude of vars derived from d corresponds to predictive value, thus direct match
                            fd += ed
                            fm += em  # accumulates difference and match between ip and all prior ips in extended rng
                            se_[j] = (ipd, fd, fm)
                            _fd += ed  # accumulates difference and match between ip and all prior and subsequent ips in extended rng
                            _fm += em
                            sub_mP, sub_mP_ = form_pattern(1, sub_mP, sub_mP_, _ip, _fd, _fm, rdn+1, rng, i, L)
                        se_ = sub_mP_  # ders replaced with sub_mPs: spans of pixels that form same-sign m

                    dP_.append( ( pri_sd, Ld, Id, Dd, Md, sr, se_ ) )
                    sd, Ld, Id, Dd, Md, sr, se_ = 0, 0, 0, 0, 0, 0, []

                Ld += 1; Id += ip; Dd += id; Md += im; se_.append( ( ip, id, im ) )
                dP = sd, Ld, Id, Dd, Md, sr, se_
            e_= dP_

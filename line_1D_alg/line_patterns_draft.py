...
def cross_comp(frame_of_pixels_):  # converts frame_of_pixels to frame_of_patterns, each pattern may be nested
    ...
    for y in range(init_y, Y):  # y is index of new line pixel_, a brake point here, we only need one row to process
        ...
        Pm_ = form_P_(None, dert_, rdn=1, rng=1, fPd=False)  # None: no rootP
...
def form_P_(rootP, dert_, rdn, rng, fPd):  # accumulation and termination
    # initialization:
    P_ = []
    x = 0
    ...
    # print(logs_2D.shape)
    return P_  # intra_Pm_ forms P.sublayers instead, including sub_Ps in 1st sublayer?

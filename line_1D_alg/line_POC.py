...
def form_pattern(dderived, P, P_, pri_p, d, m, rdn, rng, x,
                 X):  # accumulation, termination, and recursive comp within exclusive mP ( dP
        ...
        if not pri_s:  # negative mP forms dP_ that replaces e_, then is evaluated for recursion within dPs
                ...
                if (pri_sd != sd and i > 0) or i == L:
                    if Ld > rng * 2 and Dd > ave_M * rdn:  # comp range increase within e_, rdn (redundancy) is incremented per comp recursion
                        ...
                        for j in range(1, Ld + 1):  # bilateral comp between consecutive dj s
                            ...
                            md = min(dj,
                                     _dj) - ave_m  # evaluation of md: magnitudes derived from d correspond to predictive value, thus m=min
                            fdd += dd  # bilateral difference and match between ds
                            fmd += md
                            if j > 1:
                                mdP, mdP_ = form_pattern(1, mdP, mdP_, _dj, fdd, fmd, rdn + 1, rng, j, Ld)
            ...
            if L > rng * 2 and -M > ave_M * rdn:  # comp range increase within e_, rdn (redundancy) is incremented per comp recursion
                ...
                for i in range(rng, L + 1):  # comp between rng-distant pixels, also bilateral, if L > rng * 2?
                    ...
                    if dderived:
                        em = min(ip,
                                 _ip) - ave_m  # magnitude of vars derived from d corresponds to predictive value, thus direct match
                    else:
                        em = ave_d - abs(
                            ed)  # magnitude of brightness has low correlation with stability, thus match is defined through d
                    fd += ed
                    ...
                    _fm += em
                    if i >= rng * 2:
                        sub_mP, sub_mP_ = form_pattern(dderived, sub_mP, sub_mP_, _ip, _fd, _fm, rdn + 1,
                                                                    rng, i, L)
...
def cross_comp(frame_of_pixels_):  # postfix '_' denotes array name, vs. identical name of its elements
    ...
    for y in range(ini_y + 1, Y):
        ...
        P_ = [];
        P = 0, 0, 0, 0, 0, 0, []  # pri_s, L, I, D, M, r, ders_ # initialized at each line
        ...
        for x, p in enumerate(
            ...
            for index, (pri_p, fd, fm) in enumerate(ders_):
                ...
                if index < max_index:
                    ...
                    P, P_ = form_pattern(0, P, P_, pri_p, fd, fm, 1, min_rng, x,
                                         X)  # forms mPs: spans of pixels with same-sign m

            ders_.appendleft((p, back_fd,
                              back_fm))  # new tuple with initialized d and m, maxlen displaces completed tuple from rng_t_

...
def form_P_(dert_, fdP):  # pattern initialization, accumulation, termination
    ...
    for p, d, m, uni_d in dert_[ini_dert:]:
        ...
        L += 1;
        I += p;
        M += m  # accumulate params with bilateral values
        if fdP:
            D += uni_d  # value of comp uni_d
        else:
            D += d
        P_dert_ += [(p, d, m, uni_d)]
        _sign = sign
...
def intra_P(P_, fdP, fid, rdn, rng):  # evaluate for sub-recursion in line P_, filling its sub_P_ with the results
    ...
    r_deep_sub_, d_deep_sub_ = [], []  # sub__hierarchy extension feedback per intra_P: d_sub_ if fdP, else r_sub_,
    ...
    for sign, rLL, dLL, L, I, D, M, dert_, r_sub_, d_sub_ in P_:
        ...
        if fdP:  # P = dP: d sign match is partial d match, precondition for der+, or in -mPs to avoid overlap
            if (abs(D) > ave_D * rdn) and L > 3:  # cross-comp uni_ds:
                ...
                tmp_r_sub_, tmp_d_sub_ = intra_P(sub_dP_, True, True, sub_rdn + 1.2, rng)  # deep layers feedback
                d_sub_ += [rsub + dsub for rsub, dsub in zip_longest(tmp_r_sub_, tmp_d_sub_, fillvalue=[])]  # fillvalue is None by default
                # this is wrong, we need to splice same-layer sub_P_s into single d_sub_ hierarchy
                tmp_r_sub_, tmp_d_sub_ = intra_P(sub_mP_, False, fid, sub_rdn + 1.2, rng + 1)
                r_sub_ += [rsub + dsub for rsub, dsub in zip_longest(tmp_r_sub_, tmp_d_sub_, fillvalue=[])]
            ...
            if M > ave_M * rdn and L > 4:  # kernel = range * 2 + 1: 3, 5, 9
                ...
                tmp_r_sub_, tmp_d_sub_ = intra_P(sub_dP_, True, True, sub_rdn + 1.2, rng)  # deep layers feedback
                d_sub_ += [rsub + dsub for rsub, dsub in zip_longest(tmp_r_sub_, tmp_d_sub_, fillvalue=[])]
                # this is wrong, we need to splice same-layer sub_P_s into single d_sub_ hierarchy
                tmp_r_sub_, tmp_d_sub_ = intra_P(sub_mP_, False, fid, sub_rdn + 1.2, rng + 1)
                r_sub_ += [rsub + dsub for rsub, dsub in zip_longest(tmp_r_sub_, tmp_d_sub_, fillvalue=[])]
        ...
        rLL[:] = [len(r_sub_)]
        dLL[:] = [len(d_sub_)]
        r_deep_sub_ = [rsub + dsub for rsub, dsub in zip_longest(r_deep_sub_, r_sub_, fillvalue=[])]
        d_deep_sub_ = [rsub + dsub for rsub, dsub in zip_longest(d_deep_sub_, d_sub_, fillvalue=[])]

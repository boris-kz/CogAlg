...
def pre_comp(typ, e_, Ave, rng):  # pre-processing for comp recursion within pattern

    Ave += ave  # filter accumulation compensates for redundancy of fv overlap
    X = len(e_)

    olp, vP_, dP_ = 0, [], []   # olp: overlap between:
    vP = 0, 0, 0, 0, 0, [], []  # pri_s, I, D, V, incr_vf, t_, olp_
    dP = 0, 0, 0, 0, 0, [], []  # pri_sd, Id, Dd, Vd, incr_df, d_, dolp_
    ...
    if typ:  # comparison range increment within e_ = t_ of vP
        ...
        for x in range(rng+1, X):
            ...
            recursive_comp(x, p, pri_p, fd, fv, vP, dP, vP_, dP_, olp, X, Ave, rng)

    else:  # comparison derivation incr within e_ = d_ of dP (not tuples per range incr?)

        pri_d = e_[0]  # no deriv_incr while rng < min_rng, only more fuzzy
        fd, fv = 0, 0
        ...
        for x in range(1, X):
            ...
            fd, fv, vP, dP, vP_, dP_, olp = \
            recursive_comp(x, d, pri_d, fd, fv, vP, dP, vP_, dP_, olp, X, Ave, rng)
...
def form_P(typ, P, alt_P, P_, alt_P_, olp, pri_p, fd, fv, x, X, Ave, rng):
    ...
    pri_s, I, D, V, incr_f, e_, olp_ = P  # debug: 0 values in P?
    ...
    if x > rng + 2 and (s != pri_s or x == X - 1):  # P is terminated and evaluated
        ...
        if typ:
            if len(e_) > rng + 3 and pri_s == 1 and V > Ave + ave_V:  # minimum of 3 tuples
                incr_f = 1  # range increase flag
                e_.append(pre_comp(1, e_, Ave, rng))  # comparison range incr within e_ = t_

        else:
            if len(e_) > 3 and abs(D) > Ave + ave_D:  # minimum of 3 ds
                incr_f = 1  # derivation incr flag
                rng = 1  # consecutive-d comp
                e_.append(pre_comp(0, e_, Ave, rng))  # comp derivation incr within e_ = d_

        P = type, pri_s, I, D, V, incr_f, e_, olp_
        P_.append(P)  # output to level_2
        # print ("type:", type, "pri_s:", pri_s, "I:", I, "D:", D, "V:", V, "incr_f:", incr_f, "e_:", e_, "olp_:", olp_)
        ...
        olp, I, D, V, incr_f, e_, olp_ = 0, 0, 0, 0, 0, [], []  # initialized P and olp
    ...
    if typ:
        e_.append((pri_p, fd, fv))  # inputs for inc_rng comp are tuples, vs. pixels for initial comp
    else:
        e_.append(fd)  # prior fds of the same sign are buffered within dP

    P = pri_s, I, D, V, incr_f, e_, olp_

    return P, alt_P, P_, alt_P_, olp  # alt_ and _alt_ are accumulated per line


def recursive_comp(x, p, pri_p, fd, fv, vP, dP, vP_, dP_, olp, X, Ave, rng):
    ...
    v = m - Ave        # relative match (predictive value) between consecutive pixels
    ...
    vP, dP, vP_, dP_, olp = form_P(1, vP, dP, vP_, dP_, olp, pri_p, fd, fv, x, X, Ave, rng)
    # forms value pattern vP: span of pixels forming same-sign fv s

    dP, vP, dP_, vP_, olp = form_P(0, dP, vP, dP_, vP_, olp, pri_p, fd, fv, x, X, Ave, rng)
...
def comp(x, p, it_, vP, dP, vP_, dP_, olp, X, Ave, rng):  # pixel is compared to rng prior pixels
...
def frame(Fp_):  # postfix '_' denotes array name, vs. identical name of its elements
    ...
    min_rng = 3  # fuzzy comp range

    global ave; ave = 63
    global ave_V; ave_V = 63 * min_rng  # min V for initial incremental-range comp(t_)
    global ave_D; ave_D = 63 * min_rng  # min |D| for initial incremental-derivation comp(d_)

    Ave = ave * min_rng  # initial min match for positive vP inclusion, += ave per recursion

    for y in range(Y):
        p_ = Fp_[y, :]   # y is index of new line p_

        rng, x, olp, vP_, dP_ = min_rng, 0, 0, [], []  # initialized at each level
        vP = 0, 0, 0, 0, 0, [], []  # pri_s, I, D, V, incr_vf, t_, olp_
        dP = 0, 0, 0, 0, 0, [], []  # pri_sd, Id, Dd, Vd, incr_df, d_, dolp_
        ...
        for x in range(1, X):  # cross-compares consecutive pixels
            ...
            comp(x, p, it_, vP, dP, vP_, dP_, olp, X, Ave, rng)

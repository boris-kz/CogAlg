...
def rebuild_blobs(frame):
    ...
    for index, blob in enumerate(frame[9]):  # Iterate through blobs
        for s, seg in enumerate(blob[1]):  # Iterate through segments
            y = seg[7]
    ...
    for (p, d, m), ders2_ in zip(ders1_, ders2__):  # pixel comp to rng _pixels in ders2_, summing dy and my
        ...
        for (_p, _d, fdy, _m, fmy) in ders2_:  # vertical derivatives are incomplete; prefix '_' denotes higher-line variable
            ...
            if index < max_index:
                ...
                for i in range(4):
                    P[i], P_[i], buff_[i], _P_[i], frame[i] = form_P(ders, x, P[i], P_[i], buff_[i], _P_[i], frame[i], i)
            index += 1
...
def form_P( ders, x, P, P_, buff_, hP_, frame, typ = 0 ):
...
def form_segment(hP, frame):
    ...
    if y == rng * 2 + 1 + ini_y:  # scan_P_ of the 1st line converts each hP into initialized blob segment:
        hP[0] = list(_P[2:11])
        hP += 0, [(_P, 0)], [_P[0], 0, 0, 0, 0, 0, 0, _P[1], hP[3], y - rng - 1, [hP], 1, 0, 0, 0, 0]  # initialize blob, min_y = current y
        hP[3] = ave_x
    else:
        if len(hP[2]) == 1 and hP[2][0][1] == 1:  # hP has one fork: hP[2][0], and that fork has one root: hP
            ...
            Ls, Is, Ds, Dys, Ms, Mys, alt_Ders, alt_Dirs, alt_Boths = hP[2][0][0]
            hP[2][0][0] = [Ls + L, Is + I, Ds + D, Dys + Dy, Ms + M, Mys + My, \
            ...
            hP[0] = params  # seg parameters
            hP += 0, [(_P, 0)], [_P[0], 0, 0, 0, 0, 0, 0, _P[1], hP[3], y - rng - 1, [hP], 1, 0, 0, 0, 0]
...
def form_blob(term_seg, frame, y_carry=0):
    ...
    if not blob[11]:  # if remaining_roots == 0: blob is terminated and packed in frame
        s, L, I, D, Dy, M, My, min_x, max_x, min_y, root_, remaining_roots, xD, alt_Der, alt_Dir, alt_Both = blob
        ...
        # Sort term_seg based on their vertical coordinate
        root_.sort( key = segment_sort_by_height )

        frame[9].append(((s, L, I, D, Dy, M, My, alt_Der, alt_Dir, alt_Both, min_x, max_x, min_y, term_seg[7]), root_, xD))
...
def image_to_blobs(image):
    ...
    for i in range(4):
        hP_ = _P_[i]
        while hP_:
            hP, frame[i] = form_segment(hP_.popleft(), frame[i])
            frame[i] = form_blob(hP, frame[i])

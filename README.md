...
Pattern is a proximity cluster of matching inputs, where match is defined as atomic compression. Here is an informal [description](https://en.wikipedia.org/wiki/Pattern). Pattern usually denotes covariance: a match of derivatives (transformations) across a set of inputs. In my terms this is a higher-derivation pattern, secondary to pattern and derivatives formed by cross-comparison of primary inputs. It’s covered by my definition: input may consist of any set of variables. But cross-comparison should proceed with incremental distance between and derivation order of comparands.
...
Basic comparison is inverse arithmetic operation between single-variable comparands, of incremental power: Boolean, subtraction, division, etc. Each order of comparison forms miss or loss: XOR, difference, ratio, etc., and match or similarity, which can be defined directly or as inverse deviation of miss. Direct match is compression of represented magnitude by replacing larger input with corresponding order of miss between the inputs: Boolean AND, min. input in comp by subtraction, integer part of ratio in comp by division, etc. 

These direct similarity measures work if input intensity represents some stable physical property, which anti-correlates with variation. This is the case in tactile but not in visual input: brightness doesn’t correlate with inertia or invariance, dark objects are just as stable as bright ones. Thus, initial match in vision should be defined indirectly, as inverse deviation of variation in intensity. 1D variation is difference, ratio, etc., while multi-D comparison has to combine them into Euclidean distance and gradient, same as in edge detectors.

Resulting patterns represent match and miss, as well as dimensions and distances: external match and miss. The latter are separate parameters: value of representation = precision of what * precision of where. All parameters are evaluated for cross-comp among patterns, forming match and miss per parameter, summed into match and miss per pattern. Which determine clustering into higher-level patterns (much ballyhooed causality is technically a spatio-temporal connectivity clustering, not a distinct concept). 

So, the number of parameters per pattern may multiply with each level of search. Comparison is selectively incremental in distance and derivation per pattern, then in composition and parameterization per level. Selection is by match per pattern vs. higher-level average match. There are also external filters, starting with pixel size and maximal comparison distance. Quantization order (bit, integer, float...) of such internal and external filters corresponds to the order of comparison, and value of each filter is updated by feedback.

There should be a unique set of operations added per level of search, hence a singular in “cognitive algorithm”. 
Final algorithm will consist of first-level operations, + recursive increment in operations per level. The latter is a meta-algorithm that extends the working level-algorithm, to handle derivatives added to current inputs. Resulting hierarchy is a pipeline: patterns are outputted to the next level, forming a new level if there is none. As long as there are novel inputs, higher levels will discover longer-range spatio-temporal and then conceptual patterns. 
...
All unsupervised learning is some form of pattern discovery: input comparison and clustering. I do both lateraly: among inputs within a level, while statistical learning does it vertically: between layers of weighted summation. Weight adjustment by “error” from comparison is effectively soft clustering: modulated inclusion or exclusion of subsequent inputs into next output. So, vertical weighting and summation is primary to lateral comparison. This is a basic conceptual flaw: local lateral cross-correlation should precede distant vertical one.

Neural Nets is a type of statistical learning, which I think can be considered a version of [centroid clustering](https://en.wikipedia.org/wiki/Cluster_analysis#Centroid-based_clustering). Basic ANN is a multi-layer perceptron: each node weighs the inputs at synapses, then sums and thresholds them into output. This normalized sum of inputs is their centroid. Output of the top layer is compared to some template, forming an error. Stochastic Gradient Descent then backpropagates the error, training initially random weights into transformations (reversed vertical derivatives) that reduce future error.

That usually means training CNN to perform some sort of edge-detection or cross-correlation (same as my cross-comp but former terms lose meaning on higher levels of search). The difference is that CNN initially performs random transforms, while my process is designed for cross-comp from the start. That’s why it can be refined by my feedback, updating the filters, which is far more subtle and selective than training by backprop. 

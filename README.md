...
I propose cross-comparison and connectivity clustering of input elements on each level of compositional hierarchy. The clusters are defined by lateral match: common subset (compression unit) of compared inputs. Such patterns are cross-compared and clustered on the next level, forming patterns of patterns and so on. Resulting hierarchy is a pipeline, with incremental quantization of input elements, as well as the range and power of comparison. Feedback adjusts hyper-parameters to filter future inputs, it doesn't directly fit the input to some template.

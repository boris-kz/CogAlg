...
This is a draft algorithm for strictly bottom-up hierarchical clustering, from pixels to eternity. My fitness function is open-ended: projected model-to-environment correspondence, vs. closed and implicitly supervised loss minimization in current ML.
Main process is [agg_recursion](https://github.com/boris-kz/CogAlg/blob/master/frame_2D_alg/agg_recursion.py), directly derived from the definition of intelligence as the ability to predict from prior or proximate input. Any prediction is interactive projection of known patterns, so it must start with pattern discovery (AKA unsupervised learning: an obfuscating negation-first term). This perspective is not novel, pattern recognition is a core of any IQ test and a main focus of ML. But to be constructive, it should be framed in terms of cross-comparison and clustering. Although clusters may overlap (be fuzzy), segmentation enables selective localized processing and ultimately scalability.

In statistical ML, generalization and pattern discovery are side effects of randomization and brute-force fitting, not an explicit objective. That includes Neural Nets, neuromorphic or artificial:
- Hebbian Learning is local weight adjustment by input / output coincidence: a binary version of direct similarity between input and activation (normalized sum of weighted inputs). Each neuron becomes a fuzzy centroid-based cluster of its synapses: activation is effectively a centroid of weighted inputs. 
- Deep Learning adjusts the weights by backprop of decomposed error: inverse similarity to the target layer. Basic ANN: [multi-layer perceptron](https://towardsdatascience.com/what-the-hell-is-perceptron-626217814f53) or [KAN](https://towardsdatascience.com/kolmogorov-arnold-networks-kan-e317b1b4d075), performs lossy stochastic chain-rule curve fitting. As with Hebbian learning, this is a prototype-based clustering: each node in hidden layer learns to represent a fuzzy cluster of input features. It doesn't look like clustering because all nodes share the same "receptive field", but that only means the clusters are fully fuzzy. Such learning is distributed over all layers, fitting to the whole target layer rather than a single local activation. 
...
Framing backprop as fuzzy prototype-based clustering, and reasoning as a connectivity-based clustering (each inference is an edge), my process is reversed:
first simpler edge-tracing, which then seeds incrementally more complex centroid formation and expansion. This obviates all forms of randomization, replacing it with exploration of immediately meaningful structures found in real input.   
...
- Clustering cycle is incrementally sparse and fuzzy expansion, from connectivity to centroid based.

This scheme is self-contained, there is no preprocessing, and all operations are derived from the above principles. Such conceptual integrity provides the confidence to invest in highly complex design, without hand-to-mouth trial and error that dominates ML.

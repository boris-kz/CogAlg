...
This is a draft algorithm for strictly bottom-up hierarchical clustering, from pixels to eternity. It's tentatively implemented in [agg_recursion](https://github.com/boris-kz/CogAlg/blob/master/frame_2D_alg/agg_recursion.py), derived from the definition of intelligence as the ability to predict (project) from prior or proximate input. Any prediction (reasoning) is interactive projection of known patterns, hence the primary process must be pattern discovery. AKA unsupervised learning: an obfuscating negation-first term. This perspective is not novel, pattern recognition is a core of any IQ test, and a main focus of ML. I posit that the most constructive framing for pattern discovery is clustering. The clusters may overlap (be fuzzy) to any degree, but effective segmentation enables localized processing, which is critical for scalability.
...
- generative cross-comp, forming match and miss, with incremental range and/or incremental derivation,
- compressive clustering, by connectivity for nodes or correlation for links, pair-wise or group-wise in centroid clusters,
- downward feedback to scale filters and input coordinates, after pattern projection and cross-comp of the projections, 
- upward feedforward and code extension by process cross-comp and clustering, ordered by prior coordinate feedback.

The first two stages are vaguely similar to self-attention and feed-forward in transformers, but with dual hierarchy of derivatives and nodes. Such nesting allows for semantically differentiated local processing and sparse message-passing, which should be far more efficient and interpretable than constantly pumping of all data through all nodes. Connectivity clustering is among the oldest methods in ML, but I think my scheme has uniquely consistent criteria to scale in the complexity of discoverable patterns:
- Links represent both similarity and difference of connected nodes.
- Similarity is defined as compression (not dot product), while difference is intrinsically negative, it borrows from co-projected similarity.

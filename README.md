...
All unsupervised learning is some form of pattern discovery: input comparison and clustering. I do both lateraly: among inputs within a level, while statistical learning does it vertically: between layers of weighted summation. Weight adjustment by “error” from comparison is effectively soft clustering: modulated inclusion or exclusion of subsequent inputs into next output. So, weighting and summation is primary to comparison, which I think is basic conceptual flaw: vertical (long-range) correlation has a priority over lateral (local) one.

Neural Nets is a type of statistical learning, which I think can be considered a version of [centroid clustering](https://en.wikipedia.org/wiki/Cluster_analysis#Centroid-based_clustering). Basic ANN is a multi-layer perceptron: each node weighs the inputs at synapses, then sums and thresholds them into output. This normalized sum of inputs is their centroid. Output of top layer is compared to some template, forming an error. Stochastic Gradient Descent then backpropagates the error, training initially random weights into transformations (reversed vertical derivatives) that reduce future error.

That usually means training CNN to perform some sort of edge-detection or cross-correlation: same as my cross-comp but less descriptive. The difference is that CNN initially performs random transforms, while my process is designed for cross-comp from the start. That’s why it can be refined by feedback updating the filters, which is far more subtle and selective than training. So, I have several problems with basic process in ANN:

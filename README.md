...
My alternative is learning patterns via primary lateral comparison, vs. secondary vertical comparison in NNs. Below I describe hierarchically recursive algorithm of search for incrementally complex patterns. Higher levels also feedback deviations of past cross-match: hyper-parameters to filter future inputs. Next I extend comparison to ANN and BNN.
...
I propose lateral comparison-first connectivity-based hierarchical clustering, where elements of input vector are cross-compared at original resolution. Then the elements are clustered into patterns by the value of resulting match and miss: separate similarity and variance. Match here is a measure of compression of a subset common between comparands. Thus, pattern is a generalized (compressed) representations of its elements. Output patterns become higher-level input elements, where cross-comp and clustering cycle repeats. Resulting hierarchy is a pipeline, with incremental quantization of inputs, as well as range and power of comparison. 

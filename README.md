...
- Pose parameters are derived by CNN layers. The first convo-layer performs cross-correlation with edge detection kernel, similar to my model, but higher convo-layers only increase kernel area. In my scheme, lateral cross-comparison is among encapsulated parameters of lower-layer clusters, followed by clustering into higher-level patterns. There may be vertical comparison to short-cuts across the hierarchy, but only to speed-up comparison that would happen after sequential propagation of input through intermediate levels.

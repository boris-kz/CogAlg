...
- Network-centric focus on generic nodes is confusing hardware with algorithm. There is no such distinction in the brain: neurons have no substrate for local memory or differentiated algorithm. They have to distribute both across a network, with huge associated delays and connection costs. But in computers, parallelization is a speed vs. efficiency trade-off, useful only for complex and semantically isolated data structures. Such structures are patterns: sets of parameters with co-occurring values, especially those of “what” and “where”. This is similar to neural ensemble, but these parameters are compared together, thus should be “encapsulated” in local memory. 

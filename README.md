...
The first two stages are vaguely similar to self-attention and feed-forward in transformers, but with dual hierarchy of derivatives and nodes. Such nesting allows for semantically differentiated local processing and sparse message-passing, which can be far more efficient and interpretable.
Considering backprop a fuzzy prototype-based clustering, and reasoning a connectivity-based clustering (each inference is an edge), my sequence is reversed.
It starts with simple lateral edge-tracing (there is no cluster-level representation yet), which then seeds incrementally more complex centroid formation with connectivity clusters. This seeding with already meaningful structures obviates all forms of randomization.
...
- Clustering cycle has incrementally sparse and fuzzy expansion stages, from connectivity to centroid based.

To contrast this with statistical ML, I think it's very helpful to frame all learning in terms of comparison and clustering. For example:
- Hebbian Learning is local weight adjustment by input-to-output coincidence: a binary version of direct similarity between input and activation. Activation is an effective centroid of weighted inputs, so each neuron becomes a fuzzy centroid-based cluster of its synapses. 
- Deep Learning adjusts the weights by backprop of decomposed error: inverse similarity to the target layer. Basic ANN: [multi-layer perceptron](https://towardsdatascience.com/what-the-hell-is-perceptron-626217814f53) or [KAN](https://towardsdatascience.com/kolmogorov-arnold-networks-kan-e317b1b4d075), performs lossy stochastic chain-rule curve fitting. As with Hebbian learning, it's a prototype-based clustering: hidden-layer nodes learn to represent a fuzzy cluster of their input features. But the prototype is the whole target layer vs. single local activation, and fitting is distributed over multiple layers. All nodes share the same "receptive field", but with different weights, similar to fully-fuzzy multi-layer version of [FCM](https://www.geeksforgeeks.org/machine-learning/ml-fuzzy-clustering/). 

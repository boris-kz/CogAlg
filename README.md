...
Higher clustering orders are increasingly centroid-based, but still sparse, segmenting the model and enabling scalability via selective local processing. There is no direct parameter training, the feedback only adjusts hyperparameters. And there is no randomization, it's replaced by empirical seeding on all levels.
...
- Similarity is defined as compression, not dot product. The difference is intrinsically negative, it only borrows from co-projected similarity.

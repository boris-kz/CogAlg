...
I think the process should be reversed: first cross-comparison of atomic inputs, then their summation into match-defined patterns (clusters): lateral [connectivity-based clustering](https://en.wikipedia.org/wiki/Cluster_analysis#Connectivity-based_clustering_(hierarchical_clustering)) vs. vertical statistical fitting in NNs. This cross-comp and clustering is recursively hierarchical, forming patterns of patterns and so on. Resulting compositional hierarchy is an indefinitely extended pipeline, where higher levels will likely reorder the input according to some critical parameters (eigenvalues). This reframing will result in [spectral clustering](https://en.wikipedia.org/wiki/Spectral_clustering). Feedback adjusts hyper-parameters to filter future inputs, vs. fitting them to templates. No training, just learning. 

...
Pattern is a proximity cluster of matching inputs, where match is defined as atomic compression. Here is an informal [description](https://en.wikipedia.org/wiki/Pattern). Pattern usually implies covariance: a match of derivatives across a set of inputs that form them. In my terms this is a higher-derivation pattern, secondary to pattern and derivatives formed by cross-comparison of primary inputs. It’s included in my definition: the inputs may consist of any set of variables. But cross-comparison should proceed with incremental distance between and derivation order of comparands.

First-level comparands are sensory inputs at the limit of resolution: adjacent pixels of video or equivalents in other modalities. All symbolic data is these primary modalities encoded by some prior cognitive process. To discover meaningful patterns, symbols must be decoded before cross-comparison. The difficulty of decoding is exponential with the level of encoding: a start with raw sensory input is by far the easiest to implement (part 0).
...
Design of this algorithm has to be driven theoretically, generality makes it too complex and slow for simple test problems. Which is probably why such schemes are not actively explored.

...
The alternative is [connectivity-based clustering](https://en.wikipedia.org/wiki/Cluster_analysis#Connectivity-based_clustering_(hierarchical_clustering)), where the first step is input cross-comparison at original resolution. Most modern methods use some lateral connectivity. CNN is edge-detection at the bottom, same as cross-comp within kernels, but with trained weights. [Transformers](https://www.quantamagazine.org/researchers-glimpse-how-ai-gets-so-good-at-language-processing-20220414/) and [graph NNs](https://towardsdatascience.com/transformers-are-graph-neural-networks-bca9f75412aa) use lateral embeddings, but they are also learned by backprop, while I think they should be defined by connectivity clustering. Similar positional encoding was explored in Hinton's [Capsule Networks](https://medium.com/ai%C2%B3-theory-practice-business/understanding-hintons-capsule-networks-part-i-intuition-b4b559d1159b) 
...
This process is very complex and deeply structured, no way it could have evolved naturally. The design is utterly decontextualized, testing before it’s complete is next to useless because the code is supposed to be recursive. Which is probably why no one seems to work on such methods. But once the code is done, there is no need for interminable glacial and opaque training, my feedback only adjusts hyperparameters.

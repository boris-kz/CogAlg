...
- comparison by division increases match to *= integer part of ratio and reduces miss to fractional part of ratio
...
- It’s a generative form of learning: neurons evolved for stimulus-to-response conversion, via weighed input summation. Hence, learning is secondary, driven by input-to-feedback comparison. Summation is a loss of resolution, which makes learning exponentially more coarse with each added layer. Hence, forward-feedback cycle must be repeated tens of thousands of times, too expensive to scale without supervision or task-specific reinforcement. My model is lateral comparison-first, immediately learning correlations. Feedback only adjusts filters that segment these correlations into patterns, for selective recursively deeper cross-comparison.  

- Both initial weights and sampling that feeds SGD are randomized, which is a zero-knowledge option. But we do have prior knowledge for raw data in real space-time: proximity predicts similarity, thus search should proceed with incremental distance and input composition. Also driven by random variation are methods like RBM, GAN, and VAE. Variation in my model is not random, it is pattern projection by co-derived miss: projected input = input - (d_input * d_coordinate) / 2.

- SGD minimizes error (top-layer miss), which is quantitatively different from maximizing match: compression. And that error is to some specific template, while my match is summed over all accumulated input. All inputs represent environment, thus have positive value. But then they are packed (compressed) into patterns, which have different range and precision, thus different representational value per relatively fixed record cost.

- Representation is fully distributed, with huge associated delays and connection costs. The brain has no alternative: there is no substrate for local memory or program in the neurons. But parallelization in computers is a simple speed vs. efficiency trade-off, useful only for complex semantically isolated nodes. Such nodes are patterns, encapsulating a set of co-derived “what” and “where” parameters. This is similar to neural ensemble, but parameters that are compared together should be localized in memory, not distributed across a network.
...
- All versions of CapsNet perform input summation first and Hebbian-type learning second. That learning is basically vertical comparison of “squashed” length of vectors, between each input and the output. That length indicates probability of some predefined capsule (I think similarity is a better term). Vertical comparison drives dynamic routing, recursively re-clustering the inputs. This is far more coarse than my lateral comparison first.
(Stacked Capsule Autoencoders do have lateral segmentation on the first layer, but not on higher layers)

- Pose parameters are derived by CNN layers. The first convo-layer performs cross-correlation with edge detection kernel, similar to my model, but comparison / detection in higher CNN layers of CapsNet is also vertical and discontinuous. I have lateral cross-comparison and clustering on all layers, deriving a pose per layer of a pattern. There may be vertical comparison to short-cuts across the hierarchy, but only to speed-up comparison which would happen after sequential propagation of input through intermediate levels.
...
My default clustering is exclusive segmentation: each element (child) belongs to only one cluster (parent). Fuzzy clustering is selective to inputs with value above the cost of adjusting for overlap. Cross-comparison involves multiple inputs, thus value is defined per cluster, and may include value of match between clusters.

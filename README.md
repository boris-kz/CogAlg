...
This scheme is a strictly pixels-up hierarchical connectivity-based clustering, theoretically derived to be scalable in both scope and complexity of potential input flow, initially video. Derived from my definition of general intelligence as cognitive ability, ultimately the ability to predict. That includes planning, which is technically a self-prediction. Any prediction is interactive projection of known patterns, hence primary process must be pattern discovery (AKA unsupervised learning: an obfuscating negation-first term). This perspective is not novel, pattern recognition is a core of any IQ test. 

Pattern recognition is also a default mode in Neural Nets, but they work indirectly, in a very coarse statistical fashion. Basic NNs, such as [multi-layer perceptrons](https://towardsdatascience.com/what-the-hell-is-perceptron-626217814f53), perform lossy stochastic chain-rule fitting. Each node sums weighted inputs, outputs normalized sum, then adjusts the weights in proportion to similarity between input and output. In Deep Learning adjustment is by error (inverse similarity) via backprop, in Hebbian Learning adjustment is by local coincidence: binary version of direct similarity. 

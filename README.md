...
That means higher cross-comp should be selective for high core_similarity + contour_variance (borrowed from average local cross-similarity). These "complemented" clusters are inherently discontinuous, so centroid clustering should be complementary to connectivity clustering:
 - connectivity clustering is a generative phase, forming new derivatives and structured composition levels, 
 - centroid clustering is a compressive phase, reducing multiple similar patterns to a single exemplar.

...
It is an inherently statistical method: inputs are summed within samples defined by initially random weights. Weighted inputs propagate through the network, then are compared to top-layer template forming an error, which then backpropagates through the network to train weights into meaningful values by Stochastic Gradient Descent. This process is comparison-last, vs. my comparison-first. Which makes training too coarse to scale without supervision or task-specific reinforcement, especially as the nets get deeper. And this cycle must be repeated thousands of times because summation is horribly lossy. 

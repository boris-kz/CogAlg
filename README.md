...
For popular introductions to cognition-as-prediction perspective see [On Intelligence](https://numenta.com/resources/on-intelligence/) by Jeff Hawkins and [How to Create a Mind](https://www.amazon.com/How-Create-Mind-Thought-Revealed/dp/0670025291/ref=cm_cr_pr_product_top). But on a technical level, they and most everyone use neural nets, which work in very coarse fashion. Basic ANN is [multi-layer perceptron](https://towardsdatascience.com/what-the-hell-is-perceptron-626217814f53), which I think is best conceptualized as a type of fuzzy [centroid-based clusterer](https://en.wikipedia.org/wiki/Cluster_analysis#Centroid-based_clustering). It sums weighted inputs, normalizes the sum into output (centroid), then adjusts the weights in proportion to coincidence or match  between input and output, locally or via backprop. 

Modern ANNs combine such vertical training with lateral embeddings: cross-correlations within input vector. CNN is edge-detection at the core, basically weighted pixel cross-comparison within kernels. Graph NNs [Graph NNs](https://towardsdatascience.com/transformers-are-graph-neural-networks-bca9f75412aa) embed lateral edges between nodes, and popular [transformers](https://www.quantamagazine.org/researchers-glimpse-how-ai-gets-so-good-at-language-processing-20220414/) can be seen as a variation of GNN. Similar positional encoding was explored in Hinton's [Capsule Networks](https://medium.com/ai%C2%B3-theory-practice-business/understanding-hintons-capsule-networks-part-i-intuition-b4b559d1159b).But all such lateral embeddings are still trained through destructive backprop: indiscriminate summation first, meaningful comparison last. 

Primary summation degrades resolution of the whole learning process, exponentially with the number of layers. Hence a ridiculous number of backprop cycles is needed to fit inputs into meaningful representations. Most practitioners agree that the process is stupid, we and evolution hit on it only for the sake of simplicity. 
I think comparison should be primary, followed by summation in patterns defined by resulting match: a lossless component of compression. Which is a fitness function of GI, and patterns are the units of selection. 

This is lateral cross-comparison followed by connectivity-based clustering, vs. secondary vertical comparison and centroid-based clustering in statistical ML. My feedback only adjusts hyper-parameters to filter future inputs, it doesn’t directly fit input to some template. Below I describe hierarchically recursive algorithm of search for incrementally complex patterns, then extend comparisons to ANN and BNN.
...
I propose compositional hierarchy, with lateral cross-comparison and connectivity clustering on each level. Clusters / patterns are defined by the value of resulting match and variance, where match is a common subset of comparands. Thus, pattern is a generalized representations of its elements, with compression of those common subsets. Output patterns are higher-level inputs, where cross-comp and clustering cycle repeats. Which forms a pipeline of levels, with incremental quantization of inputs, range, and power of comparison. 
...
So, a pattern is a cluster of matching items, where match is compression achieved by replacing the items with derivatives, see “Comparison” section below. In more common usage, pattern is a recurring set or order of items, to me those are 2nd order patterns. If the items co-vary: they don't match but their derivatives do, then they form higher-derivation pattern, where the elements are the derivatives. 
Pattern composition should start with atomic items, such as pixels or their equivalents in other modalities. Symbolic data is merely a product of their encoding by some prior cognitive process. To discover meaningful patterns, these symbols must be decoded before cross-comparison. The difficulty of decoding is exponential with the level of encoding, thus a start with raw sensory input is by far the easiest to implement (part 0).

...
Less coarse (more selective) are Capsule Networks, recently introduced by Geoffrey Hinton et all. But they are largely ad hock, still work-in-progress, and also use layers of CNN. Neither ANN nor CapsNet is theoretically derived. I outline my approach below and then compare it to ANN, biological NN, CapsNet, and clustering.
...
Currently the most successful method is CNN, which computes match as a product: input * kernel (weights), adjusted by activation function. But kernels are formed by incredibly stupid process, starting with random weights and adjusted by very coarse trial and error, delayed by hidden layers. This is understandable in the brain: neurons are fixed at birth, vs. grown on demand, so they can only learn by adjusting their connections. But in software, generating new nodes to locally represent learned content should be far more efficient.

...
Comparison forms patterns, defined here as nearest-neighbor graphs of matching inputs. In other words, patterns are proximity clusters of input patterns with above-average match to nearest neighbor, at below-maximal comparison distance. These filters: average match and maximal distance, are higher-level feedback. Proximity clustering is critical because it can encode pose: coordinates and dimensions, the “where" part of representation and prediction. Value of prediction = precision of what * precision of where (part 2). 
    
Patterns are represented by summed parameters derived from individual comparisons: internal match and miss, + external continuity and distance. These parameters are selectively cross-compared between inputs, forming match and miss per parameter. So, number of parameters per pattern may multiply on each level. Match and miss per parameter are combined into match and miss per pattern, which define higher patterns.
 
Cross-comparison within a level is selectively incremental in input depth, distance, and derivation, while pattern parameterization and composition are incremental between levels. Which implies a unique set of operations per level, hence a singular in “cognitive algorithm“. Resulting hierarchy is a dynamic pipeline: terminated patterns are outputted for comparison on the next level, hence a new level must be formed for a pattern terminated by current top level. Which continues as long as the system receives novel inputs.
...
Higher levels should discover longer-range spatio-temporal and then conceptual patterns, of indefinite abstraction. To avoid combinatorial explosion, search is selective per dimension and element of input pattern.
As distinct from autoencoders, there is no need for decoding: comparison and clustering is done on each level. 
But this detailed comparison-first process is too complex and slow to pay off in simple test problems, which is probably why such schemes are not actively explored.

Many readers note disconnect between abstractness of this outline and specificity of current code. That specificity is due to our space-time continuum: each level of search must follow proximity and represent distances in each dimension. So, all processing is specific to input dimensionality, which requires a lot of detail.
Another complaint is that I don't use mathematical notation, but it simply doesn't have the flexibility to express deeply conditional, incrementally complex process.
...
Such learning is local, within each node. But it is still a product of vertical comparison: centroid is a higher order of composition than individual inputs. This comparison across composition drives all statistical learning, but it destroys positional information at each layer. Compared to autoencoders: principal backprop-driven unsupervised learning technique, Hebbian learning lacks the decoding stage. Decoding decomposes hidden layers, to equalize composition orders of output and compared template.

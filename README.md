...
Proposed algorithm is a comparison-first alternative to deep learning. It's a version of [kNN](https://en.wikipedia.org/wiki/K-nearest_neighbors_algorithm), except that each class is defined by input pattern, not separately. That means it’s neither statistical nor neuromorphic, with complex self-contained comparison, derivation, and evaluation process allowing for fuzzy clustering. 

The process is designed to search for patterns of increasing composition in extended pipeline. Each level of this pipeline cross-compares inputs, then clusters them into patterns by resulting match. These patterns are inputs for the next level of search (cross-comparison), which forms patterns of patterns, etc. Higher levels also feed back average deviations of past match, combined across levels, to filter future inputs.

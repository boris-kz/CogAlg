...
I propose cross-comparison and  connectivity clustering of input elements on each level, forming compositional hierarchy of patterns. Patterns are defined by lateral match: common subset of compared inputs, thus a subject of compression. Such patterns are cross-compared and clustered on the next level, and so on. Resulting hierarchy is a pipeline, with incremental quantization of input elements, as well as the range and power of comparison. Feedback adjusts hyper-parameters to filter future inputs, vs. fitting the input to some template.

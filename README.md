...
So, pattern is a cluster of matching items, where match is compression achieved by replacing the items with differences, see “Comparison” section below. In more common usage, pattern is a recurring set or sequence of items, those are 2nd order patterns in my terms. If the items co-vary: they don't match but their derivatives do, then they form higher-derivation pattern, where the elements are the derivatives. 

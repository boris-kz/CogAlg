...
Pattern is a set of matching inputs, where match is compressibility per input, see “Comparison” section below. Sometimes pattern is defined as some reoccurring item or group of items, but in my terms those are elements of a pattern. If the items co-vary: they don't match but their derivatives do, then the elements are those derivatives. This is covered by my definition: item may consist of any set of variables. But lower-derivation and shorter-range cross-comparison must be done first, starting with primary patterns of atomic inputs.
...
Such learning is "vertical": weight adjustment by feedback from higher layers of weighted summation. The feedback is originally an error from top-layer comparison to some template. Weight adjustment is soft clustering: modulated inclusion or exclusion of subsequent inputs into next output. So, weighted summation precedes comparison, and which means that comparand is immediately severely degraded. I think that's a conceptual flaw: lateral cross-correlation should form the output, which would then determine degradation of the input.

...
def comp_pixel(pixel_, _pixel_, _P_, frame):
    " Comparison of adjacent pixels to compute gradient "
    dert__ = frame[8]
    P_ = deque()
    buff_ = deque()
    p = _pixel_[0]  # input pixel
    x = 0
...
def form_P(dert, x, x_stop, P, P_, buff_, hP_, frame):
    ...
    Dx += dx  # lateral D
    Dy += dy  # vertical D
    dert_.append(dert)  # derts are buffered for intra_blob recursion
...
def scan_P_(P, P_, _buff_, hP_, frame):
    ...
    while _min_x <= max_x:  # while x values overlap between P and _P
        ...
        roots = hP[4]
        _P = hP[3][-1][0]
        _min_x, _max_x = _P[1]  # 1st_x, lst_x
...
def form_segment(hP, frame):
    ...
    ave_x = max_x - (params[0] - 1) // 2  # extra-x L = L-1 (1x in L)
...
def image_to_blobs(image):
    ...
    y = Y - 1 + min_coord    # frame ends, merge segs of last line into their blobs:

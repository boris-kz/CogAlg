...
class cl_frame(object):
    ''' frame class hold references and buffers essential for comparison and clustering operations.
        ...
        - blob_map: boolean map of local frame per blob, = True inside the blob, = False outside
        provide ways to manipulate blob' derts.
    '''
    def __init__(self, dert__, blob_box = None, num_params = 7, copy_dert = False):
...
class cl_P(cl_frame):
    ...
    def __init__(self, x0 = 0, num_params = 5, sign=-1):
        ...
        self.sign = sign  # normally 0 or 1, -1 for unknown sign
        self.box = [x0]   # initialize box with x_1st
        self.params = [0] * num_params  # initialize params with zeroes: [L, I, G, Dx, Dy] for initial comp (default)

    def terminate(self, xn, y):
        self.box += [xn, y]  #  P ends, complete boundaries with x_end and y
        return self

    def localize(self, x0, y0):  # convert frame coordinates to blob box coordinates (translation)
...
class cl_segment(cl_P):
    ''' segments are contiguous same-sign Ps, with only one P per line.
        ...
        - box = [x_start, x_end, y_start, y_end]: bounding box of segment
        - params: initially = [L, I, G, Dx, Dy], with G being the core variable.
    ...
    extend_box = (min, max, min, max)  # used by extend_box() method, shared by all instances of blob class and subclasses
    ...
    def accum_P(self, P):
        ...
        self.accum_params(P.params).extend_box(P.box[:2])   # accum params and extend x-boundaries
        self.Py_.append((P, xd))                # buffer P into Py_
        self.roots = 0                          # reset roots
        return self

    def extend_box(self, box):
        ''' extend boundaries of a box:
        ...
        for i in range(len(box)):
            self.box[i] = self.extend_box[i](self.box[i], box[i])
        return self

    def terminate(self, y_end):  # complete boundaries of terminated segment with y_end
...
class cl_blob(cl_segment):
    ''' blobs are 2D patterns of continuous same-sign pixels.
        ...
        - blob_box: to determine which dert in dert__ belongs to current blob
    ...
    def term_segment(self, segment, y):
        ...
        self.accum_params(segment.params).extend_box(segment.box[:2])  # accum params and extend x-boundaries
    ...
    def merge(self, fork):  # merge fork blob into this blob

        if not fork is self:   # check if other blob is this blob
            self.accum_params(fork.params).extend_box(fork.box)  # accum params and extend boundaries
            ...
            for seg in fork.segment_:  # for all segments in fork' blob
                self.accum_segment(seg) # segments of fork blob is buffered into this blob
    ...
    def localize(self, frame):
        ...
        if copy_dert:  # localize inner structures:
            blob_box = np.zeros(shape=(yn - y0, xn - x0), dtype=bool)
        for seg in self.segment_:
            ...
            for P, xd in seg.Py_:
                ...
                if copy_dert:
                    ...
                    blob_box[y, x_1st:x_last] = True   # pixels inside blob box = True
...
def form_P(x, y, s, dert, P, P_):
    ...
    if s != pri_s and pri_s != -1:
        P.terminate(x, y)  # P.boundaries = [x0, xn, y]
        P_.append(P)
...
def form_segment(y, P_, frame):
    " Convert P to segment or add to higher-line segment, merge blobs "
    ...
    while P_:
        ...
        if not fork_:  # if no higher-line connection:
            ...
            if len(fork_) == 1 and fork_[0].roots == 1:  # single connection with higher-line segment, which has single lower-line connection: roots == 1
                ...
                blob.extend_box(P.box[:2])  # extend x-boundaries

...
'''
    ...
    Minimal unique unit of 2D gradient is vertex of rightward and downward derivatives per pixel.

    Vertex gradient is computed as average of these two equally representative sample derivatives. 
...
def ycomp(t_, _t_, fd, fv, _x, y, X, Y, r, a, vP_, dP_, _vP_, _dP_,
    ...
    for x in range(1, X):  # compares vertically consecutive tuples, resulting derivatives end with 'y' and 'q':
        ...
        if x > r + 2 and (s != pri_s or x == X - 1):  # if vq sign miss or line ends, vP is terminated
            ...
            if y > 1:  # comb_P() or separate comb_vP() and comb_dP()?
               ...
               i_vPP, _P_, alt_P_, next_P_ = comb_P(vP, len(vP_), _vP_, _dP_, r, A, _x, x, y, Y, _P_, next_P_)
               vP = vP, i_vPP
        ...
        if x > r + 2 and (sd != pri_sd or x == X - 1):  # if dq sign miss or line ends, dP is terminated
            ...
            if y > 1:  # comb_P() or separate comb_vP() and comb_dP()?
               ...
               i_dPP, _P_, alt_P_, next_P_ = comb_P(dP, len(dP_), _dP_, _vP_, r, A, _x, x, y, Y, _P_, next_P_)
               dP = dP, i_dPP
...
def comb_P(P, nP, _P_, alt_P_, r, A, _x, x, y, Y, P_, next_P_):  # _x: x of _P displaced from _P_ by last comb_P
    ...
    while x >= _x:  # P scans over remaining _P_ while there is some horizontal overlap between P and next _P
        ...
        _s, _ix, _x, _w, _I, _D, _Dy, _M, _My, _Q, _r, _e_, _olp, _olp_, _root_ = _P  # _olp_ for PM re-eval?
        ...
        for i in range(len(olp_)):  # olp_ s are accumulated into olP_, for potential adjustment of PM and then rdn
            ...
            ialt_P = olp_[i]; alt_P = alt_P_[ialt_P]; _PM = alt_P[0]  # indirect access to _PP in alt. vP_ | dP_
        ...
        if _x <= ix:  # _P and PP output if no horizontal overlap between _P and next P:
            ...
            PP = W, IP, DP, DyP, MP, MyP, QP, P_  # PP per _root P in _root_
            ...
            if (len(_fork_) == 0 and y > r + 3) or y == Y - 1:  # no continuation per _P, term of PP, accum of CP:
                ...
                _P = _s, _ix, _x, _w, _I, _D, _Dy, _M, _My, _Q, _r, _e_, _olp_, _fork_, _root_  # PP index per root
    ...
    return len(PP_), _P_, alt_P_, next_P_  # len(PP_) is index of PP (which may include len(CP_)), for inclusion in P

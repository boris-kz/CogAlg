...
'''   
    ...
    Pixel comparison in 2D forms lateral and vertical differences per pixel, combined into gradient. This is not novel, I design from the scratch 
    to make it organic part of hierarchical algorithm. Though it would be faster with matrix computation, I implement it sequentially for 
    consistency with accumulation into blobs: irregular and very difficult to map to matrices. Most of computation will be on higher levels anyway.
...
def vertical_comp(dert1_, rng_dert2__, _P_, frame):
    ...
    P = [0, 0, 0, 0, 0, 0, 0, 0, []]  # lateral pattern = pri_s, x0, L, I, G, M, D, Dy, der2_
    P_ = deque()
...
def form_blob(term_seg, frame, y_carry=0):
    ...
    if not blob[2]:  # if remaining_roots == 0: blob is terminated and packed in frame
        ...
        frame[8] += Ly  # +L
        frame[9].append(([s, L, I, M, D, Dy, abs_D, abs_Dy], [min_x, max_x, min_y, term_seg[7], xD, Ly], root_))
    # ---------- form_blob() end ----------------------------------------------------------------------------------------
...
ave         = 22        # |g| value that coincides with average match: mP filter
ave_da      = 15        # |d_angle| value that coincides with average match: mP filter

...
def ycomp(t_, _t_, fd, fv, _x, y, X, Y, r, a, _vP_, _dP_,
    ...
    vP_, dP_, next_vP_, next_dP_ = [],[],[],[]  # or full vP_, dP_ initialization per line, out as _vP_, _dP_?
    A = a * r
    ...
    for x in range(1, X):  # compares vertically consecutive tuples, resulting derivatives end with 'y' and 'g':
        ...
        # formation of 1D value pattern vP: horizontal span of same-sign vg s with associated vars: form_P()?
...
def Le1(f):  # last "_" denotes array vs. element, first "_" denotes higher-line array, pattern, variable
    ...
    pri_s, I, D, Dy, M, My, Vg, p_, rdn, alt_ = 0,0,0,0,0,0,0,[],0,[]  # or rdn is local to ycomp? as are all S vars?
    pri_sd, Id, Dd, Ddy, Md, Mdy, Dg, d_, drdn, dalt_ = 0,0,0,0,0,0,0,[],0,[]
    ...
    for y in range(1, Y):
        ...
        _vP_, _dP_ = \
        ycomp(t_, _t_, fd, fv, _x, y, X, Y, r, a, _vP_, _dP_,  # + vP, dP, vP_, dP_ for local accumulation?
              pri_s, I, D, Dy, M, My, Vg, p_, rdn, alt_,
              pri_sd, Id, Dd, Ddy, Md, Mdy, Dg, d_, drdn, dalt_)

        # comb_P() and cons_P2() are triggered by PP ) CP termination within ycomp()
        # form_P(p_tuple, P for cum, vP_ for term)?

        _vP_.reverse(), _dP_.reverse()  # for pop(), at the start of each line, at the end of ycomp
